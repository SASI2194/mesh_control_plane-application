...
| :--- | :--- | :--- |
| **`v4.3.0`** | **Current Release** | **Major Release: Automatic RouterOS Hardware Master AP Failover, YAML Configuration (`config/failover.yaml`), Rank-Staggered Probing & Station-Bridge State Lock**: <br/>• **Automated Master AP Failover Engine ([dashboard/server.py](file:///home/nvidia/meshcontrolplane/dashboard/server.py))**: Continuously monitors Master AP health across all 9 mesh nodes. Elects next online candidate when primary Master AP goes offline, and automatically preempts failovers when primary Master AP recovers.<br/>• **Configurable Failover Registry & Rank-Staggered Probing ([config/failover.yaml](file:///home/nvidia/meshcontrolplane/config/failover.yaml))**: Externalized device priority hierarchy (`device_priority`), rank-staggered probe switching intervals (Priority 1: 30s, +15s per lower rank), custom interval overrides (`device_custom_intervals`), and complete device hardware registry (`device_nodes`) linking host IP and NetMetal AX radio IP (`192.168.3.8` shared across GCS switch).<br/>• **Connected Station-Bridge State Lock**: Resolved post-connection mode flapping by locking connected client nodes in `STATION-BRIDGE` mode indefinitely while link is active.<br/>• **RouterOS v7 REST API PATCH Reconfiguration ([hardware/routeros_client.py](file:///home/nvidia/meshcontrolplane/hardware/routeros_client.py))**: Reconfigures virtual slave interfaces to `station-bridge` FIRST before setting physical radio to `ap`, cleanly handling hardware TCP synthesizer resets during mode swaps. |
| **`v4.2.0`** | Prior Release | **Major Release: Automatic RouterOS Hardware Master AP Failover & REST API HTTP PATCH Reconfiguration**: <br/>• Initial release of automated Master AP failover engine and REST API HTTP `PATCH` mode swapping. |
| **`v4.1.0`** | Prior Release | **Feature & Governance Release: Master AP vs Station-Bridge Auto-Detection & Gold UI Badge Integration**: <br/>• **Master AP Auto-Detection Engine ([dashboard/server.py](file:///home/nvidia/meshcontrolplane/dashboard/server.py))**: Automatically analyzes live RouterOS 5GHz radio telemetry (`wifi2`) across all 9 mesh nodes to determine Master Access Point vs Station-Bridge roles.<br/>• **Gold Master AP UI Badges ([dashboard/public/app.js](file:///home/nvidia/meshcontrolplane/dashboard/public/app.js))**: Renders prominent golden `👑 MASTER AP` badges on Master Access Point nodes and `🔗 STATION BRIDGE` badges on client nodes across Web Telemetry Portals (`http://<node-ip>:8080`). |
| **`v4.0.0`** | Prior Release | **Major Release: NetMetal AX Hardware WiFi Radio Telemetry Panel, 3-Second Automated REST API Audit Loop, Inter-Device Zenoh Control Plane Radio Topic Broadcast (`/mesh_wifi_telemetry`) & Strict Device Telemetry Isolation**: <br/>• **NetMetal AX Hardware WiFi Radio Details Panel ([dashboard/public/app.js](file:///home/nvidia/meshcontrolplane/dashboard/public/app.js))**: Integrated live hardware radio interface details (`wifi1`, `wifi2`, `wifi2_vap`, `wifi2_vsb`, mode, band, operating frequency, RouterOS flags `MBR`, `BR`, `MBX`, `BI`, `X`) onto every device card in the Web Telemetry Portal (`http://<node-ip>:8080`).<br/>• **Automated Real-Time RouterOS REST API Audit Daemon Loop ([dashboard/server.py](file:///home/nvidia/meshcontrolplane/dashboard/server.py))**: Built non-blocking pre-emptive Basic Auth REST API hardware inspector (`_fetch_radio_interfaces()`) polling every 3 seconds for instant Winbox state updates.<br/>• **Inter-Device Zenoh Control Plane Radio Telemetry (`filtered/_mesh_wifi_telemetry/<ip>`)**: Implemented distributed Zenoh & native ROS 2 topic broadcast for sharing NetMetal AX WiFi radio status across all 9 mesh nodes in real time.<br/>• **Native ROS 2 Topic Graph Integration (`/mesh_wifi_telemetry`)**: Registered `/mesh_wifi_telemetry` in `ROSPublisherBridge` so it is 100% visible in `ros2 topic list` and echoable via `ros2 topic echo /mesh_wifi_telemetry`.<br/>• **Strict Device Telemetry Isolation & Memory Integrity**: Enforced physical host IP resolution (`_get_this_machine_ip()`) and deep-copy dictionary isolation (`copy.deepcopy`), ensuring local audit loops only update local cards, and remote broadcasts only update matching remote cards.<br/>• **RF Association Status Badge Differentiation**: Differentiated **`DISABLED`** (manually turned off in Winbox, Flag `X`) vs **`INACTIVE (NO LINK)`** (turned on, waiting for RF peer association, Flag `I`) vs **`RUNNING`** (actively connected, Flag `R`). |
