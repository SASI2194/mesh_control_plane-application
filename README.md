...
| :--- | :--- | :--- |
| **`v4.2.0`** | **Current Release** | **Major Release: Automatic RouterOS Hardware Master AP Failover, REST API HTTP PATCH Reconfiguration & Dynamic Single-Master Election**: <br/>• **Automated Master AP Failover Engine ([dashboard/server.py](file:///home/nvidia/meshcontrolplane/dashboard/server.py))**: Continuously monitors Master AP health across all 9 mesh nodes. Elects next online candidate (UGV-03) when primary Master AP goes offline, and automatically preempts failovers when primary Master AP recovers.<br/>• **RouterOS v7 REST API PATCH Reconfiguration ([hardware/routeros_client.py](file:///home/nvidia/meshcontrolplane/hardware/routeros_client.py))**: Executes HTTP `PATCH` method with Basic Auth (`/rest/interface/wifi/<id>`) to swap physical radio (`wifi2`) and virtual slave interface (`wifi2_vap`/`wifi2_vsb`) modes dynamically.<br/>• **Virtual AP Conflict Re-ordering & Hardware Socket Reset Handling**: Reconfigures virtual slave interfaces to `station-bridge` FIRST before setting physical radio to `ap`, cleanly handling hardware TCP synthesizer resets during mode swaps. |
