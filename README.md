...
| :--- | :--- | :--- |
| **`v4.1.0`** | **Current Release** | **Feature & Governance Release: Master AP vs Station-Bridge Auto-Detection & Gold UI Badge Integration**: <br/>• **Master AP Auto-Detection Engine ([dashboard/server.py](file:///home/nvidia/meshcontrolplane/dashboard/server.py))**: Automatically analyzes live RouterOS 5GHz radio telemetry (`wifi2`) across all 9 mesh nodes to determine Master Access Point vs Station-Bridge roles.<br/>• **Gold Master AP UI Badges ([dashboard/public/app.js](file:///home/nvidia/meshcontrolplane/dashboard/public/app.js))**: Renders prominent golden `👑 MASTER AP` badges on Master Access Point nodes and `🔗 STATION BRIDGE` badges on client nodes across Web Telemetry Portals (`http://<node-ip>:8080`). |
